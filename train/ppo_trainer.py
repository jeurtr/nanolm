...
from model import LlmModel, VlmModel
from nanolm.device import empty_cache

from .base_trainer import BaseTrainer
from .checkpoint import load_checkpoint, save_checkpoint, save_steps
...
class ValueModel(nn.Module):
    def __init__(self, base_model: LlmModel | VlmModel):
        super().__init__()
...
class PPOTrainer(BaseTrainer):
    ...
    def __init__(
            ...
            *,
            ...
            reward_func: Callable[[list[torch.Tensor], torch.Tensor, list[torch.Tensor | None]], list[float]],
    ...
    def _init_train_model_and_optim(self, initial_lr: float):
        ...
        if TrainerTools().parallel.is_main_process:
            for name, model in zip(['policy', 'value'], [policy_model, value_model], strict=False):
