...
from .parallel import DdpParallel, DsParallel
...
def sync_model_params(_from: nn.Module, _to: nn.Module | None, mixup_alpha: float = 1.0):
    """
...
def _get_ds_full_state_dict_on_rank0(model: nn.Module) -> dict | None:
    """
...
def _copy_params(model, target_model, mixup_alpha):
    for target_param, copy_param in zip(target_model.parameters(), model.parameters(), strict=False):
        target_param.data.mul_(1.0 - mixup_alpha).add_(copy_param.data, alpha=mixup_alpha)


def _sync_ds_model_params(_from: nn.Module, _to: nn.Module | None, mixup_alpha: float = 1.0):
    import deepspeed
...
def _sync_ddp_model_params(_from: nn.Module, _to: nn.Module | None, mixup_alpha: float = 1.0):
